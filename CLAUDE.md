...
- Auto-distribution sorts by **priority 1–10** (1 = front) with a **one-lead rotation** (`LEADS_PER_TURN = 1`) — a straight round robin: lead 1 to priority 1, lead 2 to priority 2, back to the top once everybody has had one. It was 8, then 5 (2026-09-14), then 1 (2026-09-17), each at the owner's instruction. Rotation governs incoming volume only, and **no test hardcodes the number** — they are written against the constant, or they only prove what it used to be.
