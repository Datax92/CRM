...
- **Source is a filter on the leads screen and in the dossier**, keyed on the exact string `describeLeadSource` prints (`sourceOptions`, `filterBySource`), so the filter and the row can never disagree. On the leads screen it is `?source=` beside `?filter=` and the formerly decorative sliders icon opens it; in the dossier it is `DossierFilters.source`, applied in `applyLeadFilters` before the cuts so the chip counts describe that source.
