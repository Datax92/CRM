...
- **Investment with X** (`lib/investmentWithX`, `investmentBooks` / `investmentRounds`): one book per partner, one round per row of the owner's sheet. **Net = Profit − every share column** — reproduces every complete row of the photographed sheet; a book can switch to Gross. Share columns are per book, keyed and renameable; removing one that still holds an amount is refused. The net goes into `invx_{bookId}` (kind INCOME), one transaction per round (`invx_{roundId}`) dated the return date, rewritten in place; changing the net basis re-posts every round. **The amount comes out of an account and goes back** (owner, 2026-09-16): a round's `funding` names one account (it takes the whole amount) or a split that must add up exactly; `invx_{roundId}_out_{accountId}` leaves on the date and `…_back_{accountId}` returns on the return date, both typed `INVESTMENT` so `isIncomeMovement` never counts capital as income. An edit reads the legs actually posted and moves each balance by the difference; deleting a round puts un-returned money back. No account is allowed and is how every earlier round reads.
  - **Invested means out now** (owner, 2026-09-26): the card and the hero read the amount of rounds *not yet received* (2,190,000), with what was ever put in and what is back as the note — it read 2,802,000 including two rounds already home. **A received round can be reinvested** into the same book or another, capital alone or with its net: *Reinvest* opens the ordinary round form pre-filled, taking the money from the accounts it came back to (plus the book's account for the net). The new round carries `reinvestedFrom`; the source gets `reinvestedInto` in the same transaction, and only a received round may be reinvested.
...
- **Group Income reads every account** (owner, 2026-09-26). A field can be linked to accounts (`accountIds`, or `accountKinds` so a new committee is picked up) or to a module (`sourceModules` — the hand-added *Investment with X* column defaults to `INVESTMENT_WITH_X`, so every book fills it; it sat empty while the profit went into Total Income); a named account beats a module beats a kind. Spending out of them fills it, income into an income field's accounts fills that field. Committee / Kist starts linked to Committee accounts, Investor to Investment accounts — defaults apply only to a field never saved with links. Unclaimed spending lands in the builtin **Other Account Spending** (`accountSpendOf`: posted `EXPENSE` legs not from office/personal — payroll, committee and manual spendings; reversals net), so nothing that left an account is missing (11 manual spendings, Rs 1,685,000, were on no column). **Deal Profit** is a builtin income column: each closed deal's `readPayoutSource` — the down payment, commission or remaining actually received, the pot Profit Distribution splits — **never** `received − payable`, which on a priced deal is the plot's price (one live deal read 2.8M instead of 30,000). **Every cell opens its records** (`figures.lines`, one per record, tagged with its column) and each line is correctable on the sheet via `incomeEdits` keyed by the line id (`deal_`/`office_`/`personal_` prefixes, txn ids bare); a month row opens the month's breakdown. Group Expense's spending list is read from the same lines.
...
- **Receivables & Payables** (`lib/receivableSheet`, `receivableEntries`): the owner's two sheets, in named groups. `AMOUNT PENDING` is derived; settling adds to `settled` and refuses more than is pending. **Settling moves money through the accounts** (owner, 2026-09-26; it was sheet-only before): *Received* puts a receivable's money into the account(s) chosen, *Pay back* takes a payable's out, with `PayFromAccounts`' split control (`settleSheetEntryThroughAccounts`, one transaction). Legs are `sourceModule: RECEIVABLE`, **`type: LOAN`** — a debt settling is neither income nor spending, so no rollup counts it. `accountSettled` is the part that went through an account; the typed settled figure may not go below it, and deleting a leg from the account un-settles the entry (`unpaySource`). *Sheet only* stays available from the same panel for money that never touched a company account. The 10 rows on the old Receivable screen are brought in by a button on the new one (`importLegacyReceivables` — copies and marks `migratedTo`, never deletes).
...
- **The admin's panel is an allow-list of five** (owner, 2026-09-22): **check-in · late arrival · absent · deal closed (profit to split) · a lead promoted to P2 or P1**. `lib/adminAlerts` holds it and `useNotifications` applies it to `targetRole: 'admin'` rows only — an employee's and a manager's bells are untouched. An allow-list, not a block-list, because the opposite shape leaks by default, which is how the panel became a log of everything the system does.
  - **Asked for by type, with the old page as a fallback** (2026-09-26). The admin's bell queries `type in ADMIN_ALERT_TYPES`, twenty rows, on the index `notifications (readAt, targetRole, type, createdAt desc)` — deployed and READY. It used to read sixty unread rows and keep the five types in the browser, because that index could not then be deployed; the read meter put the bell at ~560 reads on 2026-09-24. **If the typed query fails for any reason** (above all a missing or building index, which Firestore answers by refusing the query outright) the sixty-row query takes over and is filtered as before, so the bell can never be emptier than it was. The badge still reads the same list as the panel, or a bell saying 12 over an empty list reads as broken.
...
- **A person's attendance history is read from the device's copy, refreshed from the server at most every six hours; today's record is a live single document** (`useAttendance`, 2026-09-26). The history listener re-read every day on every visit to /home (~800 reads on 2026-09-24). A punch is never late — it is today's document, whose updates also land in the device copy. **A correction to a past day reaches that person's device at the next refresh**, up to six hours later. The team screen's `getTeamAttendance` reuses a saved answer for **months already over** for six hours (`savedOrLoad(…, reuseFor)`); the current month is always fresh, because that is where corrections land.
...
**Reloading the same small list is the read bill now, not its size** (2026-09-26)
- Measured over two days: outside leads, **every** heavy list was an `initial` answer — the same tens-to-hundreds of documents re-downloaded each time a screen mounted — with almost no `update`s. So the fix was sharing, not change-only sync: **a list read by more than one screen goes through `useLive`** (`lib/liveCollection`), one listener per query, kept **30 minutes** after the last screen closes (`KEEP_ALIVE_MS`, Firestore's own resume window). `dataBankFolders`, `clientFolders`, both `clientFolderLeads` hooks and `useOfficeExpenses` were moved onto it. **A raw `onSnapshot` in a list hook is the thing to look for** when a screen's reads look high.
- **One key, one query.** `useLive` builds a key's query from whichever caller arrives first, so two builders under one key silently serve one of them the wrong list. The whole company's expenses have exactly one definition, `ALL_EXPENSES_KEY` + `allExpensesQuery` (`hooks/useFinancials`), used by the income figures and the Office Expenses ledger alike.
- **A hook that loads more than its caller draws is a read leak.** `useFinancials(…, withExpenses = false)` for deals-only screens: the Directory paid for every expense on each open and drew none.
- **Server screens share a minute-long copy, and a payment never does.** `lib/server/attendanceMonth` (`readAttendanceMonth`, `readRoster`) is read by payroll and the whole-team calendar; an `attendance/` write drops it on that server (`noteWrite`). Anything whose figures become a write — `payPayrollLine` → `liveLines(month, true)` — passes `fresh` and reads the database, per the cache's own rule.
- **Read only the people shown.** `getTeamAttendance` read the whole company's range to draw one person (121 records for 23); it now asks `uid in` the wanted people on the existing `attendance (uid, dayKey)` index.
- **Don't run a fallback query over dates it cannot help.** `loadEntries` unioned a `creditUid` and an `authorUid` query over every range, so every entry since September was read twice. The `authorUid` half now covers only days up to `LAST_UNCREDITED_DAY` (2026-09-02) — measured: 12/12 entries that day lack `creditUid`, 0 of 673 since 20 Sept do.
...
### 2026-09-26 (afternoon) — sharing instead of reloading: the rest of the read cuts

*"yes do everything"* — after a read-meter walkthrough of every admin page (~1,500 reads) and the question whether everything could be stored locally with only changes read. Answer given: possible (leads already do it), not worth it for every list — a change-only sync cannot see the 63 places the server deletes, and ~40% of the walkthrough was server-built screens that device caching cannot touch. Rules under **Lessons → Reloading the same small list**.

- **Directory, deals, distribution, reports, campaigns, search, profile, money hub** read deals only (`withExpenses = false`).
- **Expenses**: one shared listener for the income figures and the Office Expenses ledger. Checked first that both sort orders see all 50 expenses (they do), so unifying loses none.
- **Keep-alive 60 s → 30 min** for every shared list.
- **Folders and Client folders** moved onto the shared listener.
- **Team attendance** reads only the people shown; **payroll and the whole-team calendar** share a minute-long month and roster; **Pay** reads fresh.
- **Employee activity** no longer reads every entry twice.
- **Not built, and why:** running totals for the server screens — the calendar draws every day and the dossier cuts per lead, so a total cannot replace their reads; a month-sheet document would, but it is a money-affecting copy kept in step by a write hook, the silent-drift shape this project keeps paying for. And no change-only sync for other lists: the data showed reloads, not changes.
- **Validation**: typecheck 0, `test` 866/866 (2 new: an attendance write drops every cached month; other writes leave them), `eslint src` 7 / 33 (baseline), `next build` compiles. Live, read-only: 673 entries since 20 Sept, none without `creditUid`; one person's month by uid reads 23 records against 121. **Not driven in a browser; savings not yet measured** — the collector is running.
- **Another session was editing this working tree at the same time** (Accounts, group finance, ledger, receivables, `clientActions.ts`, one hunk of this file). None of its files were touched here, and only this round's changes were committed.

### 2026-09-26 — three read fixes, from what the meter measured

*"yeah fix all the problems"* — the three remaining costs from the read meter's day of 2026-09-24 (23,333 reads counted, 09:00–17:00). The largest — lead lists re-downloaded on every visit, ~63% — was already fixed permanently on the 25th (every lead list syncs by delta).

- **The audit trail loads on demand, fifty at a time** (`useLeadHistory(leadId, auditLimit)`, `AUDIT_PAGE`). It was subscribed in full on every lead open and is shown on one tab; ~2,400 reads that day, and the log that grows without bound now the lane loops. Not loaded → `events: null`, so no screen prints a confident "0"; the tab count appears only once the whole trail is loaded. Load older events on the desktop pane, the phone and `LeadDetailModal`. Closed Deals no longer loads it at all — it printed its length in one hint.
- **The contact history was deliberately not paged.** About a dozen sites read `followUps.length` as the entry count — the Remark label, the next entry's kind, the day rule's inputs — and a window would relabel the oldest loaded entry "Remark". It was the smaller half (~1,100). Paging it means carrying `lead.followUpCount` into every one of those sites first.
- **Attendance** — rule under **Attendance**: history cache-first with a six-hour refresh, today live; team attendance reuses past months for six hours.
- **The bell** — rule under **Alerts**: typed query on a new index, sixty-row fallback.
- **Validation**: typecheck 0, `test` 857/857, `eslint src` 7 / 33 (identical with and without the change), `next build` compiles all 84 routes. The index was created with `npm run deploy:indexes` and is READY; the bell's typed query was run against the live project and returned 20 rows, only the admin's types. **Not driven in a browser**, and **the saving is estimated, not measured** — the read-meter collector stopped on the 25th at 22:53; restart `node scripts/readmeter-collect.mjs` and compare a working day.
...
- **Quiet hours narrowed** (`meteredFirestore`): for the admin and HR (`setQuietHoursExempt`, from `AuthContext`) **only the money lists they write to stay live** (`MONEY_LIVE` — expenses, accounts, transactions, deals, payouts, investments, receivables, the income modules); every other list they open is served from the device's copy. **Single documents are always live**; and `LIVE_IN_QUIET` keeps a person's own lists live — `leads/*/followUps`, `leads/*/events`, `personalExpenses`, `leaveRequests` — so a remark or a leave request shows at once. **Attendance history is served from the device's copy; today's own record is watched as a single document** (`useAttendance`, `attendance/{uid}_{today}`, the day re-read every minute), so a punch shows at once for one read.
- **Server-computed screens keep their last answer** (`lib/quietSaved`, `savedOrLoad`): `getPayroll`, `listSalaryProfiles`, `getTeamAttendance`, `getAttendanceSummary` and `buildActivityBreakdown` save every successful result in `localStorage` (keyed by the signed-in uid and the arguments); during quiet hours a saved copy is returned instead of asking the server, and with none it loads once and saves. A change drops the stale copy (`forgetSaved`): saving a salary, paying one, a punch, an attendance correction, closing or reopening a month.
- **A spent read allowance never shows as a loading error** (`meteredFirestore.quotaFallback`, and `getDocs`/`getDoc`): a listener or read refused with `RESOURCE_EXHAUSTED` is answered from the device's copy and the screen simply stops updating; `savedOrLoad` returns the last saved answer when the load fails. **Permanent, not tied to the quiet-hours date.** Saving still reports its failure (`QUOTA_MESSAGE`, neutral wording) — the owner asked for every error to be hidden, and a save that did not happen looking done was refused: it loses work silently.
- **The outbox** (`lib/outbox`, `lib/outboxOverlay`, until `OUTBOX_UNTIL` = 2026-09-26 12:00): a save refused *because the read allowance is spent* (`QUOTA_MESSAGE`) — `addFollowUp`, `updateFollowUp`, `setLeadStatus`, `acceptLead`, `passLead` — is kept in this browser's `localStorage`, answered as saved, and shown as made (`overlayLeads` on every lead list, `overlayFollowUps` on a lead's history, and the offer popup drops an accepted or passed lead). `startOutboxFlusher` (from `AuthContext`) sends the queue after noon, oldest first, with a fresh token: a remark keeps its real time through `occurredAt`; a refusal other than the allowance is logged and dropped. **Check in / out is deliberately not queued** — accepting a time from the device would let anybody set their own arrival; the attempt to add it was refused by the session's safety check and not pursued. Rarer saves (deals, KYC, payroll) still show `QUOTA_MESSAGE`. A browser cleared before noon loses its queue (accepted by the owner).
- **Team → Reports is out of the menus during quiet hours** (`useQuietHours`, `HIDDEN_IN_QUIET` — desktop sidebar and the phone's account sheet), and its two routes send a direct visit to Directory / Team.
