...
import { usePersonalExpenses } from "@/hooks/useAccounts";
...
import { addPersonalExpense } from "@/lib/clientActions";
...
export default function PersonalExpensePage() {
  ...
  const { records, loading: dataLoading, error: dataError } = usePersonalExpenses(range, isAdmin);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await addPersonalExpense(token, { title, description, amount: Number(amount), date });
      if (res.ok) {
  ...
  return (
    <div className="page-enter flex flex-col min-h-screen bg-background p-4 sm:p-6 lg:p-8 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Personal Expense</h1>
          <p className="text-sm text-slate-500">Track and manage personal expenses.</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm min-w-[200px]">
          <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider">Total Personal (This Period)</p>
          ...
          <tbody className="divide-y divide-slate-200">
            {visibleRecords.map((rec) => (
              <TableRow key={rec.id}>
                ...
                <TableCell numeric tone="negative">{formatNegativeMoney(rec.amount)}</TableCell>
              </TableRow>
            ))}
      ...
      <Modal isOpen={isOpen} onClose={() => !busy && setIsOpen(false)} title="Add Personal Expense">
