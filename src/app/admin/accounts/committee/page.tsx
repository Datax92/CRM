...
import { useCommitteeRecords } from "@/hooks/useAccounts";
...
import { addCommitteeRecord } from "@/lib/clientActions";
...
export default function CommitteePage() {
  ...
  const { records, loading: dataLoading, error: dataError } = useCommitteeRecords(range, isAdmin);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await addCommitteeRecord(token, { title, description, amount: Number(amount), date });
      if (res.ok) {
    ...
    <div className="page-enter flex flex-col min-h-screen bg-background p-4 sm:p-6 lg:p-8 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">Track and manage committee funds.</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm min-w-[200px]">
          <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider">Total Committee (This Period)</p>
      ...
      <Modal isOpen={isOpen} onClose={() => !busy && setIsOpen(false)} title="Add Committee Record">
