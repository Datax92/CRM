...
test('the hold: offers made the night before and that morning open from 10:40, in slots a few minutes apart', () => {
  ...
  assert.equal(laneResumesAt(pk(26, 9, 30), 'b22'), laneResumesAt(pk(25, 23, 0), 'b22'));
  // Its five minutes start at its slot, and the popup waits for it.
  const deadline = acceptDeadlineFrom(pk(26, 10, 0), FIVE_MIN, 'b22').getTime();
